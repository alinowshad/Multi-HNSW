...
class DataQuantizer {
   private:
    ...
    void
    rabitq_factor(const float*, const std::vector<PID>&, const IntRowMat&, const FloatRowMat&, float*, float*, float*, float*)
        const;
    ...
    void
    exrabitq_codes(const IntRowMat&, const FloatRowMat&, uint8_t*, ExFactor*, const float*, const std::vector<PID>&)
        const;

    void
    data_transformation(const float*, const std::vector<PID>&, const Rotator&, FloatRowMat&, IntRowMat&)
        const;

    void store_compacted_code(uint8_t*, uint8_t*) const;

    void convert_binxp_to_unpacked(const IntRowMat&, uint8_t*, const std::vector<PID>&) const;
   ...
   public:
    ...
    DataQuantizer& operator=(const DataQuantizer& other) = default;
    ...
    size_t padded_dim() const { return D; }

    size_t ex_bits() const { return EX_BITS; }
    ...
    void
    quantize(const float*, const std::vector<PID>&, const Rotator&, uint8_t*, uint8_t*, uint8_t*, ExFactor*) const;
...
void DataQuantizer::quantize(
    ...
    uint8_t* short_data,
    uint8_t* long_code,
    uint8_t* unpacked_codes,
    ExFactor* ex_factor) const {
    size_t num_points = IDs.size();  // Num of point in this cluster

    FloatRowMat XP_norm;
    IntRowMat bin_XP;

    data_transformation(data, IDs, rotator, XP_norm, bin_XP);

    if (unpacked_codes != nullptr) {
        convert_binxp_to_unpacked(bin_XP, unpacked_codes, IDs);
    }
    ...
    exrabitq_codes(bin_XP, XP_norm, long_code, ex_factor, all_factor_x2, IDs);
...
void DataQuantizer::rabitq_factor(
    ...
    for (size_t i = 0; i < num_points; ++i) {
        // distance 2 centroid
        const float* cur_data = data + this->DIM * IDs[i];
        // fac_x2[i] = dot_product_similarity(cur_data, centroid, this->DIM);
        float dist2c = 0; //fac_x2[i];  // x
#if defined(HIGH_ACC_FAST_SCAN)
        fac_x2[i] = dist2c;
#endif
        ...
        fac_ip[i] = 1.0f / o_obar;
        // fac sumxb
        int sumxb = bin_XP.row(i).sum();
        fac_sumxb[i] = static_cast<float>(sumxb);

        // error factor
        fac_err[i] = std::sqrt((1.0f - o_obar*o_obar)/(o_obar*o_obar)) * FAC_ERR;
...
void DataQuantizer::exrabitq_codes(
    ...
    for (size_t i = 0; i < num_points; ++i) {
        ...
        size_t id = IDs[i];
#if defined(HIGH_ACC_FAST_SCAN)
        ex_factor[id].xipnorm = ipnorm; //* fac_x2[i];
#else
        ex_factor[id].xipnorm = ipnorm; // * 2 * std::sqrt((double)fac_x2[i]);
#endif
        ...
        store_compacted_code(tmp_code, long_code + id * long_code_length());
    }
}
...
/**
 * @brief Convert binary matrix to unpacked bit format
 */
 void DataQuantizer::convert_binxp_to_unpacked(const IntRowMat& bin_XP, uint8_t* unpacked_codes, const std::vector<PID>& IDs) const {
    size_t num_points = bin_XP.rows();
    
    for (size_t i = 0; i < num_points; ++i) {
        size_t id = IDs[i];
        uint8_t* code_ptr = unpacked_codes + id * short_code_length();
        std::memset(code_ptr, 0, short_code_length());
        
        for (size_t j = 0; j < D; ++j) {
            if (bin_XP(i, j) == 1) {
                size_t byte_idx = j / 8;
                size_t bit_idx = j % 8;
                code_ptr[byte_idx] |= (1 << (7 - bit_idx));
            }
        }
    }
}