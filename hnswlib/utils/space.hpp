...
FORCE_INLINE __m512 cvtepu8x16_ps(__m128i values) {
    return _mm512_cvtepi32_ps(_mm512_cvtepu8_epi32(values));
}

FORCE_INLINE void fmadd_u8x16(
    __m512& acc,
    const float* __restrict__ x,
    __m128i values
) {
    acc = _mm512_fmadd_ps(_mm512_loadu_ps(x), cvtepu8x16_ps(values), acc);
}
...
FORCE_INLINE float IP32_fxu4(
    ...
    size_t num64 = D - (D & 0x3F);
    for (size_t i = 0; i < num64; i += 64) {
        __m128i packed0 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&y[i / 2]));
        __m128i packed1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&y[i / 2 + 16]));

        fmadd_u8x16(sum0, &x[i], _mm_and_si128(packed0, mask));
        fmadd_u8x16(sum1, &x[i + 16], _mm_and_si128(_mm_srli_epi16(packed0, 4), mask));
        fmadd_u8x16(sum2, &x[i + 32], _mm_and_si128(packed1, mask));
        fmadd_u8x16(sum3, &x[i + 48], _mm_and_si128(_mm_srli_epi16(packed1, 4), mask));
    }

    __m512 sum = _mm512_add_ps(_mm512_add_ps(sum0, sum1), _mm512_add_ps(sum2, sum3));
    for (size_t i = num64; i < D; i += 32) {
        __m128i packed = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&y[i / 2]));
        fmadd_u8x16(sum, &x[i], _mm_and_si128(packed, mask));
        fmadd_u8x16(sum, &x[i + 16], _mm_and_si128(_mm_srli_epi16(packed, 4), mask));
    }
...
FORCE_INLINE float IP_fxu4(
    ...
    for (size_t i = 0; i < num32; i += 32) {
        __m128i a8 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(&y[i / 2]));
        __m128i b8 = a8;
...
FORCE_INLINE float IP64_fxu6(
    ...
    for (size_t i = 0; i < D; i += 64) {
        ...
        fmadd_u8x16(sum0, &query[i], vec_00_to_15);
        fmadd_u8x16(sum1, &query[i + 16], vec_16_to_31);
        fmadd_u8x16(sum2, &query[i + 32], vec_32_to_47);
        fmadd_u8x16(sum3, &query[i + 48], vec_48_to_63);
...
//             // Load 16 elements of query vector
//             auto qq = _mm512_loadu_ps(q);
//             // Load 16 elements of centroid vector
//             // auto cc = _mm512_loadu_ps(c);
//             // auto uu = _mm512_mul_ps(_mm512_sub_ps(qq, cc), fac);
//             auto uu = _mm512_mul_ps(qq, fac);
...
inline float normalize_query16(
    ...
    // Calculate the norm from the input vector q
    float calculated_norm = 0.0f;
#if defined(__AVX512F__)
    __m512 sum = _mm512_setzero_ps();
    for (size_t i = 0; i < D; i += 16) {
        __m512 qq = _mm512_loadu_ps(&q[i]);
        sum = _mm512_fmadd_ps(qq, qq, sum);
    }
    calculated_norm = std::sqrt(_mm512_reduce_add_ps(sum));
#else
    for (size_t i = 0; i < D; ++i) {
        calculated_norm += q[i] * q[i];
    }
    calculated_norm = std::sqrt(calculated_norm);
#endif

    if (calculated_norm > eps) {
        auto sum = _mm512_setzero_ps();
        auto fac = _mm512_set1_ps(1.0 / calculated_norm);
        for (size_t i = 0; i < D; i += 16) {
            // Load 16 elements of query vector
            auto qq = _mm512_loadu_ps(q);
            // Load 16 elements of centroid vector
...
/**
 * @brief Calculate dot product similarity between two vectors
 * 
 * @param x First vector
 * @param y Second vector
 * @param L Length of vectors
 * @return float Dot product similarity
 */
FORCE_INLINE float dot_product_similarity(
    const float* __restrict__ x, const float* __restrict__ y, size_t L
) {
    float result = 0.0f;
    for (size_t i = 0; i < L; ++i) {
